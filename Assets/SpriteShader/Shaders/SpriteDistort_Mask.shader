Shader "Sprite/Distort Aura Mask" {
	Properties{
		...
		[Toggle(MASK)] _MASK("Distort Mask", Float) = 1
	}
		SubShader{
			Tags
		{
		...
		Stencil
	   {
		...
		Pass {
			...
			sampler2D _MainTex, _Distort, _MaskTexture;
			...
			half4 m = tex2D(_MaskTexture, i.uv);
#if MASK
			m = tex2D(_MaskTexture, float2(scaledUV.x + d, scaledUV.y + d));
#endif
